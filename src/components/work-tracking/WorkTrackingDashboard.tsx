...
export function WorkTrackingDashboard() {
  ...
  useEffect(() => {
    ...
    const preparedDays = prepareDays(loadDaysFromStorage(), today);
    setDays(preparedDays);
    setActiveDate(today);
    setTaskForm(createTaskFormState(today));
    setTimerRemainingSeconds(getDay(preparedDays, today).timerDuration * 60);
    setHasHydrated(true);
  }, []);
      ...
      setTimerRemainingSeconds((previous) => {
        if (previous <= 1) {
          ...
          setDays((previousDays) => {
            ...
            const day = {
              ...getDay(previousDays, sessionDate),
              tasks: getDay(previousDays, sessionDate).tasks.map((task) => ({ ...task })),
            };
            day.focusMinutes += sessionDuration;
            nextDays[sessionDate] = day;
            return nextDays;
          });
  ...
  function switchToDate(dateKey: string) {
    const preparedDays = prepareDays(days, dateKey);
    setIsTimerRunning(false);
    setDays(preparedDays);
    setActiveDate(dateKey);
    setTaskForm(createTaskFormState(dateKey));
    setTimerRemainingSeconds(getDay(preparedDays, dateKey).timerDuration * 60);
  }
  ...
  function handleCreateTask(event: React.FormEvent<HTMLFormElement>) {
    ...
    const nextTask: Task = {
      ...
      estimate: Math.max(0, Number(taskForm.estimate) || 0),
    ...
    setDays((previousDays) => {
      const nextDays = { ...previousDays };
      const day = {
        ...getDay(previousDays, activeDate),
        tasks: getDay(previousDays, activeDate).tasks.map((task) => ({ ...task })),
      };
      day.tasks.unshift(nextTask);
      nextDays[activeDate] = day;
      return nextDays;
    });
    setTaskForm(createTaskFormState(activeDate));
  }
  ...
  function updateTaskStatus(taskId: string, nextStatus: TaskStatus) {
    setDays((previousDays) => {
      ...
      const day = {
        ...getDay(previousDays, activeDate),
        tasks: getDay(previousDays, activeDate).tasks.map((task) =>
          task.id === taskId
            ? {
                ...task,
                status: nextStatus,
  ...
  function deleteTask(taskId: string) {
    setDays((previousDays) => {
      ...
      const day = {
        ...getDay(previousDays, activeDate),
        tasks: getDay(previousDays, activeDate).tasks
